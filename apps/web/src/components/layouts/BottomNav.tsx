...
function BottomNav({ onAddClick }: BottomNavProps): React.ReactElement {
  ...
  return (
    <nav className="pb-safe fixed bottom-0 left-0 right-0 z-40 border-t-4 border-bajes-black bg-white">
          ...
          if (isAdd) {
            return (
              <button
                key={item.href}
                onClick={onAddClick}
                className="-mt-7 flex h-14 w-14 items-center justify-center rounded-2xl border-4 border-bajes-black bg-bajes-yellow shadow-brutal transition-all hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-brutal-hover active:translate-x-[4px] active:translate-y-[4px] active:shadow-none"
              >
                {item.icon}
              </button>
            );
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                isActive ? "text-bajes-black" : "text-gray-400",
              )}
            >
              {item.icon}
              <span
                className={cn(
                  ...
                  isActive && "text-bajes-black",
                )}
              >
                {item.label}
              </span>
              {isActive && (
                <div className="h-1.5 w-1.5 rounded-full border border-bajes-black bg-bajes-yellow" />
              )}
            </Link>
          );
        })}
